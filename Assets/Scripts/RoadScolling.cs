...
public class RoadScolling : MonoBehaviour
{
                        ...
                        //hàm này sẽ được chạy trước các hàm như Start(), Update(), LateUpdate(), FixedUpdate(),...
    {
        ...
        //kích thước thực sự của một đối tượng trong không gian game là kết quả của phép nhân kích thước của collider và tỷ lệ scale của nó (vì đôi khi tỷ lệ scale chúng ta set khác 1)
    ...
    public void Update() // hàm này được gọi mỗi frame (khung hình)
    {
        ...
        if (backGround1.transform.position.y <= -ySize) // về bản chất thì 1 cái là vị trí, 1 cái là kích thước nhưng cả hay đều có giá trị float nên so sánh được
